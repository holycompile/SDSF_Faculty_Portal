...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_request_otp'])) {
    ...
    if (empty($username)) {
        ...
        if (!$admin) {
            $error = 'No admin account found matching that username or email.';
        } else {
            $email = !empty($admin['email']) ? $admin['email'] : 'joyobratadas.85912@gmail.com';

            // Invalidate existing unused OTPs
            $inv = $pdo->prepare("UPDATE password_reset_otps SET is_used = 1 WHERE user_type = 'admin' AND identifier = ?");
            $inv->execute([$admin['username']]);
            ...
            $ins = $pdo->prepare("
                ...
                VALUES ('admin', ?, ?, ?, ?)
            ");
            $ins->execute([$admin['username'], $email, $otp, $expiresAt]);

            // Send Email via Brevo
            $mailResult = sendOtpEmail($email, $admin['username'], $otp, 'Admin');
...
// STEP 2: VERIFY OTP AND RESET ADMIN PASSWORD
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_reset_password'])) {
    ...
    if (empty($otp)) {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE user_type = 'admin' AND identifier = ? AND otp = ? AND is_used = 0 AND expires_at > NOW()
        ...
        if (!$otpRecord) {
            ...
            $updAdmin = $pdo->prepare("UPDATE admin SET pass = ? WHERE username = ?");
            $updAdmin->execute([$newPassword, $username]);

            setFlash('success', 'Admin password updated successfully! Please log in with your new credentials.');
...
<title>Admin Password Reset — SDSF Portal</title>
...
.card-header { background: linear-gradient(135deg, #312e81 0%, #4f46e5 100%); color: #fff; padding: 30px 24px; text-align: center; }
...
.btn-submit { width: 100%; padding: 12px; border: none; border-radius: 10px; background: linear-gradient(135deg, #4f46e5, #4338ca); color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 8px; margin-top: 16px; }
...
<div class="reset-card">
    <div class="card-header">
        <div style="font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: #c7d2fe; font-weight: 700; margin-bottom: 4px;">University Administration</div>
        <h1 style="font-size: 20px; font-weight: 800; margin: 0;">Admin Password Reset</h1>
        <p style="font-size: 13px; color: #e0e7ff; margin-top: 4px;">Verified Email OTP Authentication</p>
    </div>
        ...
        <?php if ($step === 1): ?>
            ...
            <form method="POST">
                ...
                <div style="margin-bottom: 18px;">
                    <label class="form-label" for="username">Admin Username or Email</label>
                    <input type="text" name="username" id="username" class="form-input" 
                           placeholder="e.g. Tester 1, admin1 or admin email" 
                           value="<?= htmlspecialchars($username) ?>" required autofocus>
                    <div style="font-size: 12px; color: #64748b; margin-top: 5px;">
                        A 6-digit verification code will be sent to the administrator's registered email address.
                    </div>
                </div>
                ...
                <div style="text-align: center; margin-top: 20px;">
                    <a href="<?= BASE_URL ?>/index.php?tab=admin" style="color: #64748b; font-size: 13px; text-decoration: none; font-weight: 600;">
        ...
        <?php else: ?>
            ...
            <form method="POST">
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <div class="input-wrap">
                        <input type="password" name="new_password" id="new_password" class="form-input" style="padding-right: 42px;"
                               placeholder="Enter new admin password" required>
                ...
                <div style="text-align: center; margin-top: 20px; display: flex; justify-content: space-between; font-size: 12.5px;">
                    <a href="<?= BASE_URL ?>/admin/forgot_password.php?u=<?= urlencode($username) ?>" style="color: #4f46e5; text-decoration: none; font-weight: 600;">
                        &larr; Resend OTP Code
                    </a>
                    <a href="<?= BASE_URL ?>/index.php?tab=admin" style="color: #64748b; text-decoration: none; font-weight: 600;">
                        Cancel
                    </a>
                </div>
            </form>
