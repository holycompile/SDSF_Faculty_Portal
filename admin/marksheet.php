...
$teacherStmt = $pdo->query("SELECT id, name, faculty_enrollment_no FROM faculty_members WHERE status = 'active' ORDER BY name ASC");
$teachers = $teacherStmt->fetchAll(PDO::FETCH_ASSOC);

$selectedTeacherId = isset($_GET['teacher_id']) ? (int)$_GET['teacher_id'] : (int)($teachers[0]['id'] ?? 0);
...
if ($selectedTeacherId > 0) {
    ...
    $courseStmt->execute([':faculty_id' => $selectedTeacherId]);
    $teacherCourses = $courseStmt->fetchAll(PDO::FETCH_ASSOC);
}

$selectedCourseId = isset($_GET['course_id']) ? (int)$_GET['course_id'] : (int)($teacherCourses[0]['id'] ?? 0);
...
foreach ($teachers as $teacher) {
    if ((int)$teacher['id'] === $selectedTeacherId) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $selectedTeacherId = (int)($_POST['teacher_id'] ?? $selectedTeacherId);
    $selectedCourseId = (int)($_POST['course_id'] ?? $selectedCourseId);
    setFlash('error', 'Marks can only be entered or edited by the assigned teacher.');
    header('Location: ' . BASE_URL . '/admin/marksheet.php?teacher_id=' . $selectedTeacherId . '&course_id=' . $selectedCourseId);
    exit;
}
...
<head>
    ...
    <title>Admin Marksheet — SDSF Portal</title>
    ...
    <style>
        ...
        .marksheet-head { padding: 22px 22px 18px; border-bottom: 1px solid #eef2ff; background: linear-gradient(135deg, #f8f9ff, #ffffff); }
        .marksheet-header-grid { display: grid; grid-template-columns: 1.3fr 1fr 1fr auto; gap: 16px; align-items: end; }
    ...
    <div class="page">
        ...
        <?php if (empty($teachers)): ?>
            <div class="card" style="padding:28px;">
                <div class="card-title" style="margin-bottom:10px;">No active faculty found</div>
                <div class="card-sub">Register or activate faculty records to start creating marksheets.</div>
            </div>
        <?php else: ?>
            <div class="marksheet-wrap">
                <div class="marksheet-head">
                    <div class="marksheet-header-grid">
                        <div>
                            <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#4f46e5;margin-bottom:8px;">Admin Marksheet</div>
                            ...
                            <div style="margin-top:8px;color:#64748b;font-size:13px;">
                                <?= htmlspecialchars($selectedTeacher['name'] ?? '') ?> · <?= htmlspecialchars($selectedCourse['program'] ?? '') ?> · Semester <?= htmlspecialchars($selectedCourse['semester'] ?? '') ?>
                            </div>
                        </div>

                        <form method="get" action="<?= BASE_URL ?>/admin/marksheet.php" style="display:flex;gap:10px;align-items:end;margin:0;">
                            <div style="min-width:230px;">
                                <label for="teacherSelect" style="display:block;font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;margin-bottom:6px;">Teacher</label>
                                <select id="teacherSelect" name="teacher_id" class="form-select" style="width:100%;padding:11px 12px;border-radius:10px;border:1px solid #cbd5e1;background:#fff;" onchange="this.form.submit()">
                                    <?php foreach ($teachers as $teacher): ?>
                                        <option value="<?= (int)$teacher['id'] ?>" <?= ((int)$teacher['id'] === (int)$selectedTeacherId) ? 'selected' : '' ?>>
                        ...
                        <form method="get" action="<?= BASE_URL ?>/admin/marksheet.php" style="display:flex;gap:10px;align-items:end;margin:0;">
                            <input type="hidden" name="teacher_id" value="<?= (int)$selectedTeacherId ?>">
                            <div style="min-width:230px;">
                                <label for="courseSelect" style="display:block;font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;margin-bottom:6px;">Subject</label>
                                <select id="courseSelect" name="course_id" class="form-select" style="width:100%;padding:11px 12px;border-radius:10px;border:1px solid #cbd5e1;background:#fff;" onchange="this.form.submit()">
                ...
                <div style="padding:20px 22px 10px;">
                    <?php if (empty($teacherCourses)): ?>
                        <div style="padding:18px 0;color:#64748b;">No subjects are assigned to the selected teacher yet.</div>
                    <?php else: ?>
                        <form method="post" action="<?= BASE_URL ?>/admin/marksheet.php">
                            <input type="hidden" name="teacher_id" value="<?= (int)$selectedTeacherId ?>">
                                ...
                                <table class="marksheet-table">
                                    ...
                                    <tbody>
                                        <?php if (empty($students)): ?>
                                            <tr>
                                                <td colspan="7" style="padding:20px;text-align:center;color:#64748b;">No students found for this course.</td>
                                            </tr>
                                        <?php else: ?>
                                            <?php foreach ($students as $student): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input class="marks-input ct-input" data-row-id="<?= (int)$student['id'] ?>" value="<?= htmlspecialchars((string)($student['class_test_1'] ?? '')) ?>" type="text" readonly aria-label="Class Test 1 for <?= htmlspecialchars($student['student_name'] ?? '') ?>">
                                                    </td>
                                                    <td>
                                                        <input class="marks-input ct-input" data-row-id="<?= (int)$student['id'] ?>" value="<?= htmlspecialchars((string)($student['class_test_2'] ?? '')) ?>" type="text" readonly aria-label="Class Test 2 for <?= htmlspecialchars($student['student_name'] ?? '') ?>">
                                                    </td>
                                                    <td>
                                                        <input class="marks-input ct-input" data-row-id="<?= (int)$student['id'] ?>" value="<?= htmlspecialchars((string)($student['class_test_3'] ?? '')) ?>" type="text" readonly aria-label="Class Test 3 for <?= htmlspecialchars($student['student_name'] ?? '') ?>">
                                                    </td>
                                                    <td>
                                                        <input class="marks-input best-of-2" data-row-id="<?= (int)$student['id'] ?>" value="<?= htmlspecialchars((string)($student['best_of_2'] ?? '')) ?>" type="text" readonly aria-label="Best of 2 for <?= htmlspecialchars($student['student_name'] ?? '') ?>">
                                                    </td>
                                                    <td>
                                                        <input class="marks-input final-sem" value="<?= htmlspecialchars((string)($student['final_sem_marks'] ?? '')) ?>" type="text" readonly aria-label="Final Semester Marks for <?= htmlspecialchars($student['student_name'] ?? '') ?>">
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            rows.forEach(function (input) {
                input.addEventListener('input', function () {
                    ...
                    const rowInputs = document.querySelectorAll('.ct-input[data-row-id="' + rowId + '"]');
                    const values = [];
                    rowInputs.forEach(function (element) {
